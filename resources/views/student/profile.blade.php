    ...
    <div class="flex justify-center items-center min-h-screen bg-gradient-to-br from-emerald-50 via-white to-green-50 p-10">
        <div
            ...
            <div class="relative h-36 bg-gradient-to-r from-green-300 via-lime-200 to-emerald-300">
                <img src="{{ asset('storage/image/smkn2-klaten.png') }}" alt="SMK Negeri 2 Klaten"
                ...
                <div class="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2">
                    <div class="rounded-full border-4 border-white shadow-lg bg-gradient-to-tr from-lime-400 to-emerald-600">
                        <div
                            ...
                            @if ($user->profile)
                                <img src="{{ asset('storage/' . $user->profile) }}" alt="{{ $user->name }}"  class="w-full h-full object-cover rounded-full">
                            @else
            ...
            <div class="pt-20 pb-6 px-6 text-center">
                ...
                <p class="text-sm text-gray-600 mb-4">{{ $user->class->class_name }}</p>

                <div class="flex flex-col gap-2 text-left text-gray-700">
                    <div class="flex items-center gap-3">
                        <p><span class="font-semibold">NISN:</span> {{ $user->NISN }}</p>
                    </div>
                    <div class="flex items-center gap-3">
                        <p><span class="font-semibold">No Absen:</span> {{ $user->absen }}</p>
                    </div>
                    <div class="flex items-center gap-3">
                        <p><span class="font-semibold">Email:</span> {{ $user->email }}</p>
                    </div>
                    <div class="flex items-center gap-3">
                        <p><span class="font-semibold">Telepon Pribadi:</span> {{ $user->telephone }}</p>
                    </div>
                    <div class="flex items-center gap-3">
                        <p><span class="font-semibold">Telepon Orang Tua:</span> {{ $user->parents_phone }}</p>
                    </div>
